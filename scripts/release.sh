...
echo
echo "  Box update — helm upgrade in place."
echo "  Image tags no longer need a --set: they hang off Chart.AppVersion"
echo "  (v0.1.81). But a release BEFORE that may have pinned them via --set,"
echo "  and --reuse-values replays exactly those pins. Clearing them once,"
echo "  with empty values, is what the flags below do — after that they can"
echo "  be dropped. Skipping this on a pinned install updates the chart while"
echo "  the pods keep the old images: looks successful, changes nothing"
echo "  (v0.1.66, and again at v0.1.80 via the Market)."
echo
echo "  The workloads.* flags are mandatory for the opposite reason:"
echo "  --reuse-values does NOT merge the new chart defaults, so a values key"
echo "  the chart only gained in this release is simply absent and the render"
echo "  dies with: index of untyped nil (v0.1.76). Helm 3.9 on the box has no"
echo "  --reset-then-reuse-values, so pass them by hand."
echo "    scp dist/insilo-$NEW_VERSION.tgz olares@192.168.1.17:/tmp/"
echo "    ssh olares@192.168.1.17 'KUBECONFIG=/etc/rancher/k3s/k3s.yaml \\"
echo "        helm upgrade insilo /tmp/insilo-$NEW_VERSION.tgz \\"
echo "        -n insilo-kaivostudio --reuse-values \\"
echo "        --set images.frontend.tag= \\"
echo "        --set images.backend.tag= \\"
echo "        --set images.whisper.tag= \\"
echo "        --set images.embeddings.tag= \\"
echo "        --set workloads.insilo.replicaCount=1 \\"
echo "        --set workloads.insilo-backend.replicaCount=1 \\"
echo "        --set workloads.insilo-whisper.replicaCount=1 \\"
echo "        --set workloads.insilo-embeddings.replicaCount=1 \\"
echo "        --set workloads.insilo-worker.replicaCount=1'"
echo
echo "  Then verify the pods actually run the new images:"
echo "    ssh olares@192.168.1.17 'KUBECONFIG=/etc/rancher/k3s/k3s.yaml \\"
echo "        kubectl get pods -n insilo-kaivostudio \\"
echo "        -o jsonpath=\"{range .items[*]}{.metadata.name}{\\\" \\\"}{.spec.containers[*].image}{\\\"\\n\\\"}{end}\"'"
echo
echo "  Note: uploading the chart in the Market UI registers the version but"
echo "  does not by itself move a running install onto the new images."
