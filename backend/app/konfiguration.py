...
# Neben dem Audio, nicht darin: `audio/` gehört den Aufnahmen.
DATEI = Path(os.environ.get("APP_DATA_DIR", "/app/data")) / "konfiguration.json"
...
async def einsammeln(conn: asyncpg.Connection) -> dict[str, Any]:
    ...
    daten: dict[str, Any] = {
        ...
        "users": _zeilen(await conn.fetch(
            """
            select u.olares_username, u.email, u.display_name, r.role::text as role
            from public.users u
        ...
        "webhooks": _zeilen(await conn.fetch(
            """
            select url, secret, events, is_active, description
            from public.org_webhooks where org_id = $1 order by created_at
            """, org_id)),
        "vorlagen_anpassungen": _zeilen(await conn.fetch(
            """
            select template_id, system_prompt
    ...
    async with conn.transaction():
        ...
        for u in daten.get("users") or []:
            row = await conn.fetchrow(
                """
                insert into public.users (olares_username, email, display_name)
                values ($1, $2, $3)
        ...
        for w in daten.get("webhooks") or []:
            await conn.execute(
                """
                insert into public.org_webhooks (org_id, url, secret, events, is_active, description)
                values ($1,$2,$3,$4,$5,$6)
                """,
        ...
        for v in daten.get("vorlagen_anpassungen") or []:
            ...
            await conn.execute(
                """
                insert into public.template_customizations (org_id, template_id, system_prompt)
                select $1, $2, $3
                where exists (select 1 from public.templates where id = $2)
                on conflict (org_id, template_id) do nothing
                """,
                org_id, UUID(v["template_id"]), v["system_prompt"],
            )
