...
echo "  The --set flags are NOT optional: --reuse-values replays the values"
echo "  stored at install time, and those still carry the old image tags."
echo "  Without them the chart updates but the pods keep running the old"
echo "  images — looks like a successful deploy, changes nothing (v0.1.66)."
echo
echo "  The workloads.* flags are just as mandatory, for the opposite reason:"
...
echo "        --set images.frontend.tag=$NEW_VERSION \\"
echo "        --set images.backend.tag=$NEW_VERSION \\"
echo "        --set images.whisper.tag=$NEW_VERSION \\"
echo "        --set images.embeddings.tag=$NEW_VERSION \\"
